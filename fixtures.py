...
shop_id = session.execute(shops.insert().values(name='Shop 1', owner_id=1)).inserted_primary_key[0] # Adding a shop
...
product_a_id = session.execute(products.insert().values(name='Product A', description='Description A', price=100, quantity=10, shop_id=shop_id, category_id=category_a_id)).inserted_primary_key[0]
product_b_id = session.execute(products.insert().values(name='Product B', description='Description B', price=200, quantity=20, shop_id=shop_id, category_id=category_b_id)).inserted_primary_key[0]
product_c_id = session.execute(products.insert().values(name='Product C', description='Description C', price=300, quantity=30, shop_id=shop_id, category_id=category_a_id)).inserted_primary_key[0]
